...
export const recurringInvoicesAPI = {
  ...
  async generateMonthly(month: number, year: number, paymentEntityId?: number, templateIds?: number[]): Promise<{
    success: boolean;
    message: string;
    data: {
      generated: number;
    ...
    return await api.post('/recurring-invoices/generate', { month, year, paymentEntityId, templateIds });
  },
