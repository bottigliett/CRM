<?php
// File: /modules/fatture/ajax/generate_pdf.php
// Generatore PDF per fatture - Nuova grafica Studio Mismo con cattura screenshot

// Includi autenticazione e configurazione
...
try {
    ...
    // Traduci giorno della settimana in italiano
    $giorni = [
        'Monday' => 'Lunedì',
        'Tuesday' => 'Martedì', 
        'Wednesday' => 'Mercoledì',
    ...
    // Log dell'azione
    logUserAction('generate_pdf_fattura', 'success', 
        'Generato PDF fattura: ' . $fattura['numero_fattura'] . ' (ID: ' . $id . ')'
    );
...
<head>
    ...
    <style>
        * {
        ...
        /* Print styles - FONDAMENTALI per la stampa */
        @media print {
            * {
                -webkit-print-color-adjust: exact !important;
                print-color-adjust: exact !important;
            ...
            /* Mantieni tutti i colori e gli sfondi */
            .divider {
...
<body>
        ...
        <div class="invoice-container" id="invoiceContent">
            ...
            <div class="header">
                <div class="logo-section">
                    <!-- Logo come testo per evitare problemi di caricamento immagine -->
            ...
            <div class="footer-disclaimer">
                ...
                <?php if ($fattura['iva_percentuale'] == 0): ?>
    ...
    <script>
        ...
        async function generatePDF() {
            ...
            try {
                ...
                const canvas = await html2canvas(element, {
                    scale: 3, // Aumenta la risoluzione (3x)
                    useCORS: true,
                    logging: false,
                ...
                // Genera nome file
                const fileName = 'Fattura_<?= str_replace(['#', '/', ' '], ['', '_', '_'], $fattura['numero_fattura']) ?>_HD.pdf';
        ...
        // Auto-genera PDF se richiesto via parametro URL
        <?php if (isset($_GET['auto_pdf']) && $_GET['auto_pdf'] === 'true'): ?>
        window.onload = function() {
            setTimeout(() => {
                generatePDF();
            }, 1000); // Aspetta che il documento sia completamente renderizzato
        };
        <?php endif; ?>
    </script>
</body>
...
<?php
    ...
    // Aggiorna il path del PDF nel database
    $pdfPath = '/modules/fatture/pdf/fattura_' . $fattura['id'] . '.pdf';
    $stmt = $pdo->prepare("
        UPDATE fatture 
