...
// VERSIONE: 2.2 - Fix collapse + submenu (26 Aug 2025)

console.log('📋 Menu.js v2.2 caricato - Fix collapse + submenu');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // === GESTIONE SOTTO-MENU LEAD E CONTATTI ===
    ...
    if (leadContactsMenu && leadContactsSubmenu) {
        console.log('✅ Submenu Lead/Contatti trovato, aggiungo listener');
        ...
        leadContactsMenu.addEventListener('click', function(e) {
            ...
            if (sidebar?.classList.contains('collapsed')) {
                console.log('Sidebar collapsed, non apro submenu');
                return;
            }
            
            // Toggle expanded state
            const isExpanded = leadContactsMenu.classList.contains('expanded');
            console.log('Submenu click - era espanso:', isExpanded);
        ...
        if (!sidebar?.classList.contains('collapsed')) {
            ...
            if (savedExpanded === 'true') {
                leadContactsMenu.classList.add('expanded');
                leadContactsSubmenu.classList.add('expanded');
                console.log('Submenu ripristinato come espanso da localStorage');
            }
        }
        ...
        if (currentPath.includes('/lead_contatti/')) {
            ...
            leadContactsSubmenu.classList.add('expanded');
            localStorage.setItem('leadContactsMenuExpanded', 'true');
            console.log('Submenu auto-espanso perché siamo in una sottopagina');
        }
    } else {
        console.log('❌ Submenu Lead/Contatti non trovato');
    }
    ...
    console.log('✅ Menu v2.2 inizializzato con successo');
});
...
window.MenuUtils = {
    toggleSidebar: function() {
        ...
        const isCollapsed = sidebar?.classList.contains('collapsed');
        localStorage.setItem('sidebarCollapsed', isCollapsed);
        console.log('Toggle sidebar via MenuUtils');
    },
    ...
    toggleSubmenu: function() {
        ...
        if (leadContactsMenu && leadContactsSubmenu) {
            leadContactsMenu.classList.toggle('expanded');
            leadContactsSubmenu.classList.toggle('expanded');
            console.log('Toggle submenu via MenuUtils');
        }
    },
    ...
    debugSidebar: function() {
        ...
        console.log('Debug Sidebar:', {
            ...
            leadContactsMenu: leadContactsMenu,
            leadContactsMenuClasses: leadContactsMenu?.classList.toString(),
            leadContactsSubmenu: leadContactsSubmenu,
            leadContactsSubmenuClasses: leadContactsSubmenu?.classList.toString(),
            localStorage: {
                sidebarCollapsed: localStorage.getItem('sidebarCollapsed'),
                leadContactsMenuExpanded: localStorage.getItem('leadContactsMenuExpanded')
            }
        });
    }
};

console.log('📋 Menu.js v2.2 - Caricamento completato');